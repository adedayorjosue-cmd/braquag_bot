...
MAX_BRAQUAGE_PLAYERS = 6
BRAQUAGE_MIN_PLAYERS = 1
BRAQUAGE_SUCCESS_RATE = 0.70
BRAQUAGE_WIN_MULTIPLIER = 2
BRAQUAGE_LOSE_MULTIPLIER = 2
ROB_SUCCESS_RATE = 0.15
ROB_PENALTY = 1000000
COURSE_WAIT_TIME = 180
MAX_COURSE_PLAYERS = 10
COURSE_HORSES = {
...
# Dans la section configuration, modifiez/ajoutez :
BRAQUAGE_WAIT_TIME = 180  # 3 minutes (comme les courses)

# Taux de réussite selon le nombre de joueurs
BRAQUAGE_SUCCESS_RATES = {
    1: 0.05,   # 5% pour 1 joueur
    2: 0.10,   # 10% pour 2 joueurs
    3: 0.20,   # 20% pour 3 joueurs
    4: 0.45,   # 45% pour 4 joueurs
    5: 0.65,   # 65% pour 5 joueurs
    6: 0.80,   # 80% pour 6 joueurs
}# ============ BASE DE DONNÉES ============

class Database:
    """Gestion de la base de données des joueurs"""
    ...
    def load(self) -> None:
        ...
        try:
            if os.path.exists(self.filename):
                ...
                logger.info("📂 Aucun fichier de données trouvé")
                self.players = {}
...
def main() -> None:
    ...
    logger.info("🤖 Démarrage du Bot Braquage...")
